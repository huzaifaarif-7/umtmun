// ========================================================
//  ASTROLABE DIAL  —  removed; background handled by CSS
// ========================================================
(function () {
    const canvas = document.getElementById('dialCanvas');
    if (!canvas) return; // canvas removed from HTML — this block is inert
    const ctx = canvas.getContext('2d');

    let W, H, cx, cy, baseR;
    let scrollY = 0;
    let targetRot = 0;
    let currentRot = 0;
    let raf;

    // Colour palette — tuned to sit over the cosmic-clock image
    const C = {
        ring:    'rgba(80,145,215,0.40)',
        ringBrt: 'rgba(130,195,255,0.65)',
        tick:    'rgba(90,155,225,0.55)',
        tickBrt: 'rgba(160,220,255,0.80)',
        text:    'rgba(140,190,245,0.65)',
        gear:    'rgba(80,150,220,0.45)',
        glow:    'rgba(20,60,140,0.12)',
    };

    function resize() {
        W = canvas.width  = window.innerWidth;
        H = canvas.height = window.innerHeight;
        cx = W / 2;
        cy = H / 2;
        baseR = Math.min(W, H) * 0.42;
    }

    // ---- drawing helpers ----
    function ring(x, y, r, color, lw) {
        ctx.beginPath();
        ctx.arc(x, y, r, 0, Math.PI * 2);
        ctx.strokeStyle = color;
        ctx.lineWidth = lw;
        ctx.stroke();
    }

    function tickMarks(x, y, r, count, innerFrac, color, lw) {
        ctx.strokeStyle = color;
        ctx.lineWidth = lw;
        for (let i = 0; i < count; i++) {
            const a = (i / count) * Math.PI * 2 - Math.PI / 2;
            const r1 = r * innerFrac;
            ctx.beginPath();
            ctx.moveTo(x + Math.cos(a) * r1, y + Math.sin(a) * r1);
            ctx.lineTo(x + Math.cos(a) * r,  y + Math.sin(a) * r);
            ctx.stroke();
        }
    }

    function romanLabel(x, y, r, rot) {
        const numerals = ['XII','I','II','III','IV','V','VI','VII','VIII','IX','X','XI'];
        ctx.fillStyle = C.text;
        ctx.font = `${Math.max(10, baseR * 0.045)}px "Cinzel", serif`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        for (let i = 0; i < 12; i++) {
            const a = (i / 12) * Math.PI * 2 - Math.PI / 2 + rot;
            const tx = x + Math.cos(a) * r;
            const ty = y + Math.sin(a) * r;
            ctx.save();
            ctx.translate(tx, ty);
            ctx.rotate(a + Math.PI / 2);
            ctx.fillText(numerals[i], 0, 0);
            ctx.restore();
        }
    }

    // ---- single debate/speaking icon, drawn as vector line-art ----
    // Local coordinate space is roughly [-su, su] in both axes, origin centred.
    function drawDebateIcon(x, y, su, type, color) {
        ctx.save();
        ctx.translate(x, y);
        ctx.strokeStyle = color;
        ctx.fillStyle = color;
        ctx.lineWidth = Math.max(1, su * 0.16);
        ctx.lineCap = 'round';
        ctx.lineJoin = 'round';

        switch (type) {
            case 0: // Gavel
                ctx.save();
                ctx.rotate(-Math.PI / 4);
                ctx.strokeRect(-su * 0.55, -su * 0.95, su * 1.1, su * 0.5);
                ctx.beginPath();
                ctx.moveTo(0, -su * 0.45);
                ctx.lineTo(0, su * 0.9);
                ctx.stroke();
                ctx.restore();
                break;

            case 1: // Microphone
                ctx.beginPath();
                ctx.moveTo(-su * 0.35, -su * 0.55);
                ctx.lineTo(-su * 0.35, -su * 0.85);
                ctx.arc(0, -su * 0.85, su * 0.35, Math.PI, 0);
                ctx.lineTo(su * 0.35, -su * 0.55);
                ctx.arc(0, -su * 0.55, su * 0.35, 0, Math.PI);
                ctx.closePath();
                ctx.stroke();
                ctx.beginPath();
                ctx.arc(0, -su * 0.45, su * 0.55, 0.15 * Math.PI, 0.85 * Math.PI);
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(0, su * 0.05);
                ctx.lineTo(0, su * 0.85);
                ctx.moveTo(-su * 0.3, su * 0.85);
                ctx.lineTo(su * 0.3, su * 0.85);
                ctx.stroke();
                break;

            case 2: // Megaphone
                ctx.beginPath();
                ctx.moveTo(-su * 0.9, -su * 0.28);
                ctx.lineTo(-su * 0.9, su * 0.28);
                ctx.lineTo(su * 0.55, su * 0.65);
                ctx.lineTo(su * 0.55, -su * 0.65);
                ctx.closePath();
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(-su * 1.05, -su * 0.08);
                ctx.lineTo(-su * 0.9, -su * 0.08);
                ctx.moveTo(-su * 1.05, su * 0.08);
                ctx.lineTo(-su * 0.9, su * 0.08);
                ctx.stroke();
                ctx.beginPath();
                ctx.arc(su * 0.55, 0, su * 0.18, -Math.PI * 0.4, Math.PI * 0.4);
                ctx.stroke();
                break;

            case 3: // Speech bubble
                ctx.beginPath();
                ctx.roundRect
                    ? ctx.roundRect(-su * 0.85, -su * 0.65, su * 1.7, su * 0.95, su * 0.2)
                    : ctx.rect(-su * 0.85, -su * 0.65, su * 1.7, su * 0.95);
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(-su * 0.45, su * 0.3);
                ctx.lineTo(-su * 0.6, su * 0.8);
                ctx.lineTo(-su * 0.05, su * 0.3);
                ctx.closePath();
                ctx.stroke();
                break;

            case 4: // Scroll
                ctx.strokeRect(-su * 0.6, -su * 0.45, su * 1.2, su * 0.9);
                ctx.beginPath();
                ctx.arc(-su * 0.6, 0, su * 0.18, 0, Math.PI * 2);
                ctx.arc(su * 0.6, 0, su * 0.18, 0, Math.PI * 2);
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(-su * 0.3, -su * 0.15);
                ctx.lineTo(su * 0.3, -su * 0.15);
                ctx.moveTo(-su * 0.3, su * 0.15);
                ctx.lineTo(su * 0.3, su * 0.15);
                ctx.stroke();
                break;

            case 5: // Podium / lectern
                ctx.beginPath();
                ctx.moveTo(-su * 0.55, -su * 0.85);
                ctx.lineTo(su * 0.55, -su * 0.85);
                ctx.lineTo(su * 0.3, su * 0.85);
                ctx.lineTo(-su * 0.3, su * 0.85);
                ctx.closePath();
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(-su * 0.65, -su * 0.85);
                ctx.lineTo(su * 0.65, -su * 0.85);
                ctx.stroke();
                break;

            case 6: // Flag
                ctx.beginPath();
                ctx.moveTo(-su * 0.5, -su * 0.9);
                ctx.lineTo(-su * 0.5, su * 0.9);
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(-su * 0.5, -su * 0.9);
                ctx.lineTo(su * 0.6, -su * 0.55);
                ctx.lineTo(-su * 0.5, -su * 0.2);
                ctx.closePath();
                ctx.stroke();
                break;

            case 7: // Scales of justice
                ctx.beginPath();
                ctx.moveTo(0, -su * 0.9);
                ctx.lineTo(0, su * 0.55);
                ctx.moveTo(-su * 0.7, -su * 0.5);
                ctx.lineTo(su * 0.7, -su * 0.5);
                ctx.moveTo(-su * 0.7, -su * 0.5);
                ctx.lineTo(-su * 0.7, -su * 0.05);
                ctx.moveTo(su * 0.7, -su * 0.5);
                ctx.lineTo(su * 0.7, -su * 0.05);
                ctx.moveTo(-su * 0.25, su * 0.55);
                ctx.lineTo(su * 0.25, su * 0.55);
                ctx.stroke();
                ctx.beginPath();
                ctx.arc(-su * 0.7, 0, su * 0.22, 0, Math.PI);
                ctx.stroke();
                ctx.beginPath();
                ctx.arc(su * 0.7, 0, su * 0.22, 0, Math.PI);
                ctx.stroke();
                break;

            case 8: // Globe
                ctx.beginPath();
                ctx.arc(0, 0, su * 0.8, 0, Math.PI * 2);
                ctx.stroke();
                ctx.beginPath();
                ctx.ellipse(0, 0, su * 0.35, su * 0.8, 0, 0, Math.PI * 2);
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(-su * 0.8, 0);
                ctx.lineTo(su * 0.8, 0);
                ctx.stroke();
                break;

            case 9: // Quill pen
                ctx.beginPath();
                ctx.moveTo(-su * 0.6, su * 0.8);
                ctx.lineTo(su * 0.6, -su * 0.8);
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(su * 0.6, -su * 0.8);
                ctx.lineTo(su * 0.88, -su * 0.5);
                ctx.lineTo(su * 0.42, -su * 0.5);
                ctx.closePath();
                ctx.fill();
                ctx.beginPath();
                ctx.moveTo(-su * 0.6, su * 0.8);
                ctx.lineTo(-su * 0.25, su * 0.65);
                ctx.stroke();
                break;

            case 10: // Open book
                ctx.beginPath();
                ctx.moveTo(0, -su * 0.55);
                ctx.quadraticCurveTo(-su * 0.85, -su * 0.85, -su * 0.85, su * 0.6);
                ctx.quadraticCurveTo(-su * 0.4, su * 0.4, 0, su * 0.6);
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(0, -su * 0.55);
                ctx.quadraticCurveTo(su * 0.85, -su * 0.85, su * 0.85, su * 0.6);
                ctx.quadraticCurveTo(su * 0.4, su * 0.4, 0, su * 0.6);
                ctx.stroke();
                break;

            case 11: // UN-style lectern mic
                ctx.beginPath();
                ctx.moveTo(0, -su * 0.9);
                ctx.lineTo(0, su * 0.5);
                ctx.stroke();
                ctx.beginPath();
                ctx.arc(0, -su * 0.95, su * 0.22, 0, Math.PI * 2);
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(-su * 0.55, su * 0.5);
                ctx.lineTo(su * 0.55, su * 0.5);
                ctx.lineTo(su * 0.3, su * 0.85);
                ctx.lineTo(-su * 0.3, su * 0.85);
                ctx.closePath();
                ctx.stroke();
                break;
        }

        ctx.restore();
    }

    // ---- ring of debate/speaking icons (replaces the old zodiac band) ----
    function speakerIcons(x, y, r, rot, color, su) {
        for (let i = 0; i < 12; i++) {
            const a = (i / 12) * Math.PI * 2 - Math.PI / 2 + rot;
            drawDebateIcon(
                x + Math.cos(a) * r,
                y + Math.sin(a) * r,
                su,
                i,
                color
            );
        }
    }

    function gearTeeth(x, y, r, teeth, rot, color) {
        const toothH = r * 0.055;
        const toothW = (Math.PI * 2 / teeth) * 0.35;
        ctx.strokeStyle = color;
        ctx.lineWidth = 1;
        ctx.beginPath();
        for (let i = 0; i < teeth; i++) {
            const a  = (i / teeth) * Math.PI * 2 + rot;
            const a1 = a - toothW / 2;
            const a2 = a + toothW / 2;
            ctx.moveTo(x + Math.cos(a1) * r,          y + Math.sin(a1) * r);
            ctx.lineTo(x + Math.cos(a1) * (r+toothH), y + Math.sin(a1) * (r+toothH));
            ctx.lineTo(x + Math.cos(a2) * (r+toothH), y + Math.sin(a2) * (r+toothH));
            ctx.lineTo(x + Math.cos(a2) * r,          y + Math.sin(a2) * r);
        }
        ctx.stroke();
    }

    function spoke(x, y, r, count, rot, color, lw) {
        ctx.strokeStyle = color;
        ctx.lineWidth = lw;
        for (let i = 0; i < count; i++) {
            const a = (i / count) * Math.PI * 2 + rot;
            ctx.beginPath();
            ctx.moveTo(x, y);
            ctx.lineTo(x + Math.cos(a) * r, y + Math.sin(a) * r);
            ctx.stroke();
        }
    }

    function radialGlow(x, y, r, color) {
        const g = ctx.createRadialGradient(x, y, 0, x, y, r);
        g.addColorStop(0, color);
        g.addColorStop(1, 'transparent');
        ctx.fillStyle = g;
        ctx.beginPath();
        ctx.arc(x, y, r, 0, Math.PI * 2);
        ctx.fill();
    }

    // ---- draw one complete dial at (x,y), scaled by `scale` ----
    function drawDial(x, y, scale, rot, clockwise, alpha) {
        ctx.save();
        ctx.globalAlpha = alpha;
        const r = baseR * scale;
        const dir = clockwise ? 1 : -1;

        // Ambient glow
        radialGlow(x, y, r * 1.15, C.glow);

        // Outer gear ring (rotates with scroll)
        gearTeeth(x, y, r * 1.02, Math.round(60 * scale), rot * dir, C.gear);
        ring(x, y, r * 1.02, C.ring, 0.8);

        // Outer ring with major & minor ticks
        ring(x, y, r, C.ringBrt, 1.2);
        tickMarks(x, y, r, 60, 0.94, C.tick, 0.8);       // minute ticks
        tickMarks(x, y, r, 12, 0.88, C.tickBrt, 1.5);    // hour ticks

        // Roman numeral ring (counter-rotates slightly)
        romanLabel(x, y, r * 0.83, rot * dir * -0.4);

        // Debate/speaking icon band (rotates opposite to main)
        ring(x, y, r * 0.73, C.ring, 0.8);
        ring(x, y, r * 0.68, C.ring, 0.5);
        speakerIcons(x, y, r * 0.705, rot * dir * 0.6, C.text, Math.max(9, baseR * 0.05));

        // Inner tick band
        tickMarks(x, y, r * 0.68, 36, 0.94, C.tick, 0.7);

        // Mid-ring decorative
        ring(x, y, r * 0.58, C.ring, 0.8);
        tickMarks(x, y, r * 0.58, 24, 0.90, C.tick, 0.8);

        // Spokes (like the astrolabe rete)
        spoke(x, y, r * 0.55, 8, rot * dir * 0.5, C.ring, 0.7);

        // Inner gear ring (counter-rotates)
        gearTeeth(x, y, r * 0.44, Math.round(36 * scale), rot * dir * -1.3, C.gear);
        ring(x, y, r * 0.44, C.ring, 0.8);
        tickMarks(x, y, r * 0.44, 48, 0.90, C.tick, 0.6);

        // Innermost bright ring
        ring(x, y, r * 0.30, C.ringBrt, 1);
        tickMarks(x, y, r * 0.30, 12, 0.86, C.tickBrt, 1);

        // Hub dot
        ctx.beginPath();
        ctx.arc(x, y, r * 0.035, 0, Math.PI * 2);
        ctx.fillStyle = C.ringBrt;
        ctx.fill();

        ctx.restore();
    }

    // ---- main render ----
    function draw() {
        // Clear to transparent so the body's cosmic-clock-bg.jpg shows through
        ctx.clearRect(0, 0, W, H);

        // Soft radial tint in centre for depth (low opacity — image provides the base)
        const grd = ctx.createRadialGradient(W/2, H*0.45, 0, W/2, H*0.45, Math.min(W,H)*0.65);
        grd.addColorStop(0,   'rgba(12,38,90,0.22)');
        grd.addColorStop(0.5, 'rgba(8,22,55,0.10)');
        grd.addColorStop(1,   'transparent');
        ctx.fillStyle = grd;
        ctx.fillRect(0, 0, W, H);

        // Lerp rotation toward target for smooth feel
        currentRot += (targetRot - currentRot) * 0.06;

        // Idle rotation — dials turn slowly even without scrolling (~13 min per revolution)
        const idleRot = Date.now() * 0.00008;
        const rot = currentRot + idleRot;

        // --- DIAL 1: large, bottom-left (matches theme image astrolabe) ---
        drawDial(
            W * 0.12,   // x — left edge, partially off-screen
            H * 0.82,   // y — low
            0.85,       // scale
            rot,
            true,       // clockwise
            1.0
        );

        // --- DIAL 2: medium, top-left (clock face from theme) ---
        drawDial(
            W * 0.06,
            H * 0.15,
            0.55,
            rot * 1.2,
            false,   // counter-clockwise
            0.88
        );

        // --- DIAL 3: small, right (clock face right-side in theme) ---
        drawDial(
            W * 0.94,
            H * 0.28,
            0.45,
            rot * 0.8,
            true,
            0.80
        );

        raf = requestAnimationFrame(draw);
    }

    // ---- scroll listener — drives rotation ----
    function onScroll() {
        scrollY = window.scrollY || document.documentElement.scrollTop;
        const maxScroll = document.documentElement.scrollHeight - window.innerHeight;
        const progress  = maxScroll > 0 ? scrollY / maxScroll : 0;
        // Full page scroll = 1.5 full rotations
        targetRot = progress * Math.PI * 3;
    }

    window.addEventListener('scroll', onScroll, { passive: true });
    window.addEventListener('resize', () => { resize(); }, { passive: true });

    resize();
    onScroll();
    draw();
})();


// ========================================================
//  INTRO ANIMATION
// ========================================================
function createParticles() {
    const container = document.getElementById('introParticles');
    if (!container) return;
    for (let i = 0; i < 40; i++) {
        const p = document.createElement('div');
        p.className = 'particle';
        const size = Math.random() * 5 + 2;
        p.style.cssText = `
            width: ${size}px;
            height: ${size}px;
            left: ${Math.random() * 100}%;
            top: ${80 + Math.random() * 30}%;
            animation-duration: ${3 + Math.random() * 5}s;
            animation-delay: ${Math.random() * 3}s;
        `;
        container.appendChild(p);
    }
}

function runIntroAnimation() {
    const overlay = document.getElementById('intro-overlay');
    if (!overlay) return;
    createParticles();
    setTimeout(() => {
        overlay.classList.add('fade-out');
        setTimeout(() => { overlay.style.display = 'none'; }, 850);
    }, 3200);
}


// ========================================================
//  COUNTDOWN
// ========================================================
let _cdPrev = null;

function updateCountdown() {
    const eventDate = new Date('2026-09-10T09:00:00');
    const now = new Date();
    const diff = Math.max(0, eventDate - now);

    const vals = [
        { v: Math.floor(diff / 86400000),           label: 'Days'    },
        { v: Math.floor(diff / 3600000)   % 24,     label: 'Hours'   },
        { v: Math.floor(diff / 60000)     % 60,     label: 'Minutes' },
        { v: Math.floor(diff / 1000)      % 60,     label: 'Seconds' },
    ];

    const el = document.getElementById('countdown');
    if (!el) return;

    // First render — build DOM
    if (!_cdPrev) {
        el.innerHTML = vals.map(({ v, label }) => `
            <div class="countdown-item">
                <div class="countdown-num-wrap">
                    <span class="countdown-num">${String(v).padStart(2,'0')}</span>
                </div>
                <span class="countdown-label">${label}</span>
            </div>
        `).join('');
        _cdPrev = vals.map(x => x.v);
        return;
    }

    // Subsequent ticks — flip only changed digits
    const items = el.querySelectorAll('.countdown-item');
    vals.forEach(({ v }, i) => {
        if (v === _cdPrev[i]) return;
        _cdPrev[i] = v;
        const numEl = items[i]?.querySelector('.countdown-num');
        if (!numEl) return;
        numEl.textContent = String(v).padStart(2, '0');
        items[i].classList.remove('flip');
        void items[i].offsetWidth; // force reflow to restart animation
        items[i].classList.add('flip');
    });
}


// ========================================================
//  REGISTER MODAL
// ========================================================
function initRegisterModal() {
    const registerBtn = document.getElementById('registerBtn');
    const modal       = document.getElementById('modal');
    const closeModal  = document.getElementById('closeModal');
    if (!registerBtn || !modal || !closeModal) return;

    registerBtn.addEventListener('click', () => { modal.style.display = 'flex'; });

    function closeHandler(e) {
        e.preventDefault();
        e.stopPropagation();
        modal.style.display = 'none';
    }
    closeModal.addEventListener('click', closeHandler);
    closeModal.addEventListener('touchend', closeHandler);
    window.addEventListener('click', (e) => { if (e.target === modal) modal.style.display = 'none'; });
}


// ========================================================
//  HERO PARALLAX  —  content drifts up & fades as you scroll
// ========================================================
function initHeroParallax() {
    const heroContent = document.querySelector('.hero-content');
    if (!heroContent) return;
    window.addEventListener('scroll', () => {
        const y = window.scrollY;
        const vh = window.innerHeight;
        if (y < vh) {
            heroContent.style.transform = `translateY(${y * 0.18}px)`;
        }
    }, { passive: true });
}


// ========================================================
//  SCROLL REVEAL  —  sections fade in from below on scroll
// ========================================================
function initScrollReveal() {
    const targets = document.querySelectorAll('.about-section, .timer-section, .committees-section');
    if (!targets.length || !('IntersectionObserver' in window)) {
        targets.forEach(el => el.classList.add('revealed'));
        return;
    }
    const obs = new IntersectionObserver((entries) => {
        entries.forEach(e => {
            if (e.isIntersecting) {
                e.target.classList.add('revealed');
                obs.unobserve(e.target);
            }
        });
    }, { threshold: 0.12 });
    targets.forEach(el => obs.observe(el));
}


// ========================================================
//  HERO PARTICLES  —  glowing blue orbs floating upward
// ========================================================
function initHeroParticles() {
    const hero = document.querySelector('.hero-section');
    if (!hero) return;
    for (let i = 0; i < 22; i++) {
        const p = document.createElement('div');
        p.className = 'hero-particle';
        const size = Math.random() * 5 + 2;
        p.style.cssText = `
            left: ${Math.random() * 100}%;
            top: ${20 + Math.random() * 75}%;
            width: ${size}px;
            height: ${size}px;
            animation-delay: ${Math.random() * 8}s;
            animation-duration: ${9 + Math.random() * 9}s;
        `;
        hero.appendChild(p);
    }
}


// ========================================================
//  INIT
// ========================================================
document.addEventListener('DOMContentLoaded', () => {
    runIntroAnimation();
    updateCountdown();
    setInterval(updateCountdown, 1000);
    initRegisterModal();
    initHeroParallax();
    initScrollReveal();
    initHeroParticles();
});
